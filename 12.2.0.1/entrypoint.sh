	...
	'')
		...
		if [ "$(ls -A /u01/app/oracle/oradata)" ]; then
			...
			if [ "${MANUAL_DBCA}" == 'true' ]; then
				...
				su oracle -c "$ORACLE_HOME/bin/dbca -silent -createDatabase -templateName General_Purpose.dbc -gdbname orcl -sid orcl -responseFile NO_VALUE -characterSet $CHARACTER_SET -emConfiguration NONE -automaticMemoryManagement false -memoryPercentage $INIT_MEM_PST -recoveryAreaDestination NONE -databaseType MULTIPURPOSE -pdbAdminPassword oracle -sysPassword oracle -systemPassword oracle"
			fi
		fi
